...
export class CobrancaService extends NotificationCronService {
  ...
  async getClientesPendentes(
    empresaId: string,
  ): Promise<{ telefone: string; [key: string]: any }[]> {
    const propostas =
      await this.propostaRepository.findAceitasParceladas(empresaId);
    return propostas
      .filter(
        (p: any) =>
          p.devedor?.telefone && p.valorAcordado && p.parcelasAcordadas,
      )
      .map((p: any) => ({
  ...
  buildMessage(proposta: any): string {
    const valorParcela = (
      proposta.valorAcordado / proposta.parcelasAcordadas
    ).toFixed(2);
