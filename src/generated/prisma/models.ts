
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
 * This is a barrel export file for all models and their related types.
 *
 * 🟢 You can import this file directly.
 */
export type * from './models/Empresa.js'
export type * from './models/Endereco.js'
export type * from './models/Devedor.js'
export type * from './models/FaixaCriterio.js'
export type * from './models/Proposta.js'
export type * from './models/ConfigRetorno.js'
export type * from './models/Paciente.js'
export type * from './models/Consulta.js'
export type * from './models/ServicoConfig.js'
export type * from './models/ClienteOficina.js'
export type * from './models/Agendamento.js'
export type * from './commonInputTypes.js'